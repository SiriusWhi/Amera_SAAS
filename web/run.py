...
if __name__ == '__main__':
    ...
    # Make templates auto reload automatically when
    # Running locally or dev mode
    ...
    # This allows running the web application in local dev mode
    # This does not require login/auth from flask
    # This does not require a web server like nginx
    if env_name in LOCAL_ENVS:
        app.add_url_rule("/assets/<path:path>",
                        methods=['GET'], view_func=load_static_assets)
        app.add_url_rule("/lib/<path:path>",
                        methods=['GET'], view_func=load_static_lib)

        opts["reload"] = settings.get("RELOAD") or True
        opts['timeout'] = 86400
        logger.debug(opts)
